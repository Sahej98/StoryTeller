  ...
  const renderContent = () => {
    ...
    if (appState === 'auth' || appState === 'auth_check')
      ...
      if (appState === 'editor') {
        if (isMobile) {
          return (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', textAlign: 'center', padding: '2rem', boxSizing: 'border-box' }}>
                <h2 className='selection-screen-title' style={{marginBottom: '1rem'}}>Editor Unavailable on Mobile</h2>
                <p style={{ color: 'var(--secondary-text-color)', fontFamily: 'var(--body-font)', maxWidth: '400px', margin: '0 0 2rem 0', lineHeight: '1.6' }}>
                    The Story Editor has complex features that require a larger screen. Please switch to a desktop device to create and edit stories.
                </p>
                <button className='themed-button secondary' onClick={handleEditorBack}>
