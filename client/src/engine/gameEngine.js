...
export function applyEffects(state, story, effects) {
    ...
    if (effects.inventory) {
        ...
        if (effects.inventory.add) {
            const itemsToAdd = Array.isArray(effects.inventory.add) ? effects.inventory.add : [effects.inventory.add];
            itemsToAdd.forEach(item => {
                if (typeof item === 'string' && !newState.inventory.includes(item)) {
                    ...
                    // Check for lore
                    const itemDef = story.items?.[item];
        ...
        if (effects.inventory.remove) {
            const itemsToRemove = Array.isArray(effects.inventory.remove) ? effects.inventory.remove : [effects.inventory.remove];
    ...
    if (effects.flags) {
        ...
        if (effects.flags.set) {
            const flagsToSet = Array.isArray(effects.flags.set) ? effects.flags.set : [effects.flags.set];
    ...
    if (effects.relationships) {
        ...
        for (const charKey of Object.keys(effects.relationships)) {
            const change = effects.relationships[charKey];
            if (typeof change !== 'number' || change === 0) continue;
...
export function checkRequirements(reqs, gameState) {
...
export function getViewModel(story, gameState, isRevisit = false) {
    ...
    if (!currentNode) {
        console.error(`Node not found in local data: ${chapter}.${key}`);
        return { gameState, view: null };
    }
    ...
    const speakerInfo = story.characters?.[speakerKey];
